...
def updatefig(i):
    ...
    if i in plot_idx:
        plt.savefig("result/tracking_invcomp_aerial_InvComp" + str(i) + ".png")
    return im,
...
np.save("aerial_masks_InvComp.npy", masks)

##### Animation
aerial_masks = np.load("aerial_masks_InvComp.npy")
...
for i in range(len(plot_idx)):
    axarr[i].imshow(plt.imread(f"result/tracking_invcomp_aerial_InvComp" + str(plot_idx[i]) + ".png"))
