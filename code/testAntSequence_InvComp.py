...
def updatefig(i):
    ...
    if i in plot_idx:
        plt.savefig("result/tracking_invcomp_ant" + str(i) + ".png")
    return im,
...
ant_masks = np.load("ant_masks_invcomp.npy")
...
for i in range(len(plot_idx)):
    ...
    axarr[i].imshow(plt.imread(f"result/tracking_InvComp_ant" + str(plot_idx[i]) + ".png"))
