...
from SubtractDominantMotion_InvComp import SubtractDominantMotion_InvComp
...
def updatefig(i):
    ...
    if i in plot_idx:
        plt.savefig("result/tracking_4_myvideo_norm" + str(i) + ".png")
    return im,

parser = argparse.ArgumentParser()
parser.add_argument('--num_iters', type=int, default=1e3, help='number of iterations of Lucas-Kanade')
parser.add_argument('--threshold', type=float, default=1e-2, help='dp threshold of Lucas-Kanade for terminating optimization')
parser.add_argument('--tolerance', type=float, default=0.8, help='binary threshold of intensity difference when computing the mask')
...
# ##### Animation
aerial_masks = np.load("squatclean_masks_InvComp.npy")
...
for i in range(len(plot_idx)):
    axarr[i].imshow(plt.imread(f"result/tracking_4_myvideo_norm" + str(plot_idx[i]) + ".png"))
